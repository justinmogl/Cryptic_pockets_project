...
impute_vivi <- function(data, sample, replicate, grouping, condition, treatment_condition, reference_condition, intensity, cutoff_MAR, cutoff_MNAR, retain_columns = NULL){
  ...
  result <- data %>%
    ...
    ungroup() %>%
    #calculate missingness: complete as default, else overwritten with MAR or MNAR
    mutate(missingness = "complete") %>%
    #if observations are below MAR cutoff - classified as MAR
    ...
    #join to data complete by grouping (fg_id) and condition
    ...
    #if missingness is na - set to MNAR
...
DIA_raw_prot <- DIA_raw%>% filter(fg_ms2raw_quantity > 1000) %>% filter(pep_is_proteotypic == TRUE)
set.seed(123)
...
png(file ="boxplot_missingness_vs_intensity.png")
plot <- boxplot(normalised_intensity_log2~missingness, data = missingness) 
title("missingness vs intensity")
#ggsave(filename = "boxplot_missingness_vs_intensity.png",  plot = plot)
plot
dev.off()

png(file ="boxplot_missingness_vs_eg_qvalue.png")
